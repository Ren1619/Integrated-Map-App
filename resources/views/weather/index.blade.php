...
@section('content')
    ...
    <header class="glass-effect p-4 text-center shadow-lg">
        <h1 class="text-gray-800 text-3xl font-bold mb-2">🗺️Integrated Maps App</h1>
    ...
    <div class="glass-effect p-4 flex flex-wrap gap-4 justify-center items-center shadow-lg">
        <div class="flx gap-2 items-center">
        ...
        <div class="flex gap-2">
            ...
            <button class="btn-primary" id="cycleBtn" onclick="app.changeMapLayer('cycle')">🚲
                Cycle</button>
            <button class="btn-primary" id="transportBtn" onclick="appchangeMapLayer('transport')">🚚
                Transport</button>
        </div>
    </div>

    <div class="flex flex-1 h-[calc(100vh-200px)] gap-4 p-4 fle-col lg:flex-row">
        <!-- Map -->
        <div id="map" class="flex-2 lg:flex-[2] rounded-2xl shadow-2xl border-4 border-white/30 h-96 lh:h-auto"></div>

        <!-- Information Panel -->
        <di class="flex-1 flex flex-col lg:flex-col md:flex-row gap-4">
            <!-- Weather Section -->
    ...
    <script>
        class EnhancedWeatherMapApp {
            ...
            bindEvents() {
                document.getElementById('serachInput').addEventListener('keypress', (e) => {
                    if (e.key === 'Enter') {
                        this.searchLocation();
                    }
                });
            ...
            async getWeatherData(lat, lng) {
                ...
                weatherInfo.innerHTML = '<div class="text-center text-gray-600 italic"><div class="text-3xl mb-4"> ⏳ </div> Loading weather data...</div>';
                ...
                try {
                    ...
                    weatherInfo.innerHTML = \'<div class="bg-red-50 text-red-600 p-4 rounded-xl mb-4"><strong>⚠️ Error</strong><br>Could not fetch weather data. Please try again</div>\';
                }
            }
            ...
            async getNearbyPOIs(lat, lng) {
                ...
                poiInfo.innerHTML = \'<div class="text-center text-gray-600 italic"><div class="text-3xl mb-4"> 📍</div>Finding nearby places...</div>\';

                try {
                    const radius = 1000;
                    const overpassQuery = \'[out:json][timeout:25];(node["amenity"~"^(restaurant|cafe|shop|bank|hospital|pharmacy|school|fuel|hotel|tourism)$"](around:\${radius},\${lat},\${lng});node["shope"](around:\${radius},\${lat},\${lng});node["tourism"](around:\${radius},\${lat},\${lng}););out center meta;\';
                    ...
                    poiInfo.innerHTML = \'<div class="bg-red-50 text-red-600 p-4 rounded-xl mb-4"><strong>⚠️ Error</strong><br>Could not fetch nearby places. Please try again</div>\';
                }
            }
            ...
            displayPOIs(pois, centerLat, centerLng) {
                ...
                poisWithDistance.forEach(poi => {
                    ...
                    html += \'<div class="bg-blue-50 p-3 rounded-lg cursor-pointer transition-all duration-300 hover:bg-blue-100 hover:-translate-y-0.5" onclick="app.focusPOI(\${poi.lat}, \${poi.lon}"><div class="font-bold text-gray-800 mb-1">\${emoji} \${name}</div><div class="text-gray-600 text-sm mb-1">\${type}</div><div class="text-gray-500 text-xs">📍 \${distanceText} away</div></div>\';
                });
                html += '</div>';
            ...
            focusPOI(lat, lng) {
                ...
                const poiMaker = L.circleMarker([lat, lng], {
                    radius: 8,
                    fillColor: '#e74c3c',
                    color: 'white',
                    weight: 2
